...
function AuthGate() {
  ...
  return (
    <>
      <div style={{ background: '#fef3c7', color: '#92400e', fontSize: 12, padding: '6px 10px', fontFamily: 'monospace', wordBreak: 'break-all' }}>
        DEBUG — role: {role || '(none)'} | {roleDebug} | userId: {session.user.id}
      </div>
      <InventoryApp role={role || 'staff'} />
    </>
  );
}
...
function Header({ onMenu, onSignOut }) {
  return (
    <header className="rf-header">
      ...
      <div className="rf-logo-badge">RF</div>
...
.rf-logo-badge {
  ...
  font-weight: 800; font-size: 12px; color: #1e293b; flex-shrink: 0;
}
.rf-hamburger {
...
@media print {
  .rf-header, .rf-sidebar, .rf-btn-row, .rf-btn-outline, .rf-icon-btn { display: none !important; }
  .rf-main { padding: 0; }
