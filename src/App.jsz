  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      ...
      <div className="rf-shell">
        <Header onMenu={() => setSidebarOpen(s => !s)} onSignOut={signOut} role={role} />
...
// HEADER + SIDEBAR
// ============================================================
function Header({ onMenu, onSignOut, role }) {
  return (
...
// ============================================================
// SALES PAGE — read-only view combining Direct Sales + Outlet Sales
// ============================================================
function SalesPage({ fabricEntries, outletSales, outlets }) {
  const outletName = (id) => outlets.find(o => o.id === id)?.name || 'Outlet';

  const directSales = fabricEntries
    .filter(e => e.direction === 'out_sale')
    .map(e => ({ id: 'f' + e.id, date: e.date, channel: 'Direct (Factory)', party: e.party || '—', quality: e.quality, qty: e.qty, unit: e.unit, rate: e.rate, amount: Number(e.qty) * Number(e.rate || 0) }));

  const outletSalesRows = outletSales.map(s => ({ id: 'o' + s.id, date: s.date, channel: outletName(s.outletId), party: s.party || '—', quality: s.quality, qty: s.qty, unit: s.unit, rate: s.rate, amount: Number(s.qty) * Number(s.rate || 0) }));

  const all = [...directSales, ...outletSalesRows].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
  const totalQty = all.reduce((s, r) => s + Number(r.qty), 0);
  const totalValue = all.reduce((s, r) => s + r.amount, 0);
  ...
  return (
    <div>
      ...
      <div className="rf-stat-grid" style={{ gridTemplateColumns: 'repeat(2, minmax(0,1fr))' }}>
        <StatCard label="Total Fabric Sold (yd/m)" value={num(totalQty)} accent="#d4a017" icon={ShoppingCart} />
        <StatCard label="Total Sales Value" value={money(totalValue)} accent="#d4a017" icon={TrendingUp} />
      </div>
      <div className="rf-card" style={{ marginTop: 20 }}>
        <h3 className="rf-section-title">All Sales — Direct &amp; Outlet Combined</h3>
        <p className="rf-hint">Pulled live from Fabric direct sales and Outlet sales — no separate entry needed here.</p>
        {all.length === 0 ? <EmptyState text="No sales recorded yet." /> : (
          <div className="rf-table-wrap">
            <table className="rf-table">
              <thead><tr><th>Date</th><th>Channel</th><th>Customer</th><th>Quality</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead>
              <tbody>
                {all.map(r => (
                  <tr key={r.id}>
                    <td>{fmtDate(r.date)}</td>
...
// ============================================================
// PURCHASES PAGE — read-only view of Yarn purchases (direction = 'in')
// ============================================================
function PurchasesPage({ yarnEntries }) {
  const purchases = yarnEntries
    .filter(e => e.direction === 'in')
    .map(e => ({ ...e, amount: Number(e.qty) * Number(e.rate || 0) }))
    .sort((a, b) => (b.date || '').localeCompare(a.date || ''));

  const totalQty = purchases.reduce((s, r) => s + Number(r.qty), 0);
  const totalValue = purchases.reduce((s, r) => s + r.amount, 0);
  ...
  return (
    <div>
      ...
      <div className="rf-stat-grid" style={{ gridTemplateColumns: 'repeat(2, minmax(0,1fr))' }}>
        <StatCard label="Total Yarn Purchased" value={`${num(totalQty)} kg/lb`} accent="#16a34a" icon={Package} />
        <StatCard label="Total Purchase Value" value={money(totalValue)} accent="#dc2626" icon={TrendingDown} />
      </div>
      <div className="rf-card" style={{ marginTop: 20 }}>
        <h3 className="rf-section-title">All Yarn Purchases</h3>
        <p className="rf-hint">Pulled live from Yarn In entries — no separate entry needed here.</p>
        {purchases.length === 0 ? <EmptyState text="No purchases recorded yet." /> : (
          <div className="rf-table-wrap">
            <table className="rf-table">
              <thead><tr><th>Date</th><th>Supplier</th><th>Quality</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead>
              <tbody>
                {purchases.map(r => (
                  <tr key={r.id}>
                    <td>{fmtDate(r.date)}</td>
...
// ============================================================
// SETTINGS PAGE — Account, Staff, Edit Entries, App Info
// ============================================================
function SettingsPage({ allData }) {
  ...
  return (
    <div>
      <div className="rf-page-head"><h2>Settings</h2></div>
      ...
      {tab === 'entries' && <SettingsEditEntries allData={allData} />}
      {tab === 'info' && <SettingsAppInfo allData={allData} />}
    </div>
  );
}

function SettingsAccount() {
  const { session, changePassword, changeEmail } = useAuth();
  ...
  const submitEmail = async (e) => {
    ...
    else setEmailMsg({ type: 'success', text: 'Check your new email inbox to confirm the change.' });
  };

  return (
    <div className="rf-settings-grid">
      <div className="rf-card">
        <h3 className="rf-section-title">Current Account</h3>
        <p className="rf-hint" style={{ marginTop: 0 }}>Signed in as <strong>{session?.user?.email}</strong></p>
      </div>

      <div className="rf-card">
        <h3 className="rf-section-title">Change Password</h3>
        <form onSubmit={submitPassword} className="rf-form">
          ...
          {passwordMsg && <div className={passwordMsg.type === 'error' ? 'rf-settings-error' : 'rf-settings-success'}>{passwordMsg.text}</div>}
          <button type="submit" className="rf-btn-primary rf-form-submit" disabled={passwordBusy}>{passwordBusy ? 'Updating…' : 'Update Password'}</button>
        </form>
      </div>

      <div className="rf-card">
        <h3 className="rf-section-title">Change Email</h3>
        <p className="rf-hint" style={{ marginTop: 0 }}>You'll need to confirm the change from a link sent to the new address.</p>
        <form onSubmit={submitEmail} className="rf-form">
          <Field label="New Email Address">
            <input type="email" value={newEmail} onChange={e => setNewEmail(e.target.value)} required />
          </Field>
          {emailMsg && <div className={emailMsg.type === 'error' ? 'rf-settings-error' : 'rf-settings-success'}>{emailMsg.text}</div>}
          <button type="submit" className="rf-btn-primary rf-form-submit" disabled={emailBusy}>{emailBusy ? 'Sending…' : 'Update Email'}</button>
        </form>
      </div>
    </div>
  );
...
function SettingsStaff() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const rows = await api.fetchUserRoles();
        setStaff(rows);
      } catch (e) {
        setError(e.message);
      }
  ...
  const removeStaff = async (id, email) => {
    if (!window.confirm(`Remove access for ${email}? They will no longer be able to log in.`)) return;
    try {
  ...
  return (
    <div className="rf-card">
      <h3 className="rf-section-title">Staff &amp; Access</h3>
      <p className="rf-hint" style={{ marginTop: 0 }}>
        To add a brand-new staff member, create their login in Supabase → Authentication → Users first
        (this requires the Supabase dashboard, not this app). Once their account exists, they'll appear
        here automatically and you can manage their role.
      </p>
      {loading ? <EmptyState text="Loading staff…" /> : error ? <div className="rf-settings-error">{error}</div> : staff.length === 0 ? (
        <EmptyState text="No staff accounts found yet." />
      ) : (
        <div className="rf-table-wrap">
          <table className="rf-table">
            ...
            <tbody>
              {staff.map(s => (
                <tr key={s.id}>
                  <td>{s.email}</td>
                  <td>
                    <select value={s.role} onChange={e => changeRole(s.id, e.target.value)} className="rf-staff-role-select">
                      <option value="owner">Owner</option>
                      <option value="staff">Staff</option>
                    </select>
                  </td>
                  <td><button className="rf-icon-btn" onClick={() => removeStaff(s.id, s.email)}><Trash2 size={15} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function SettingsEditEntries({ allData }) {
  const [section, setSection] = useState('yarn');
  ...
  const rowsFor = (id) => {
    if (id === 'yarn') return allData.yarnEntries;
    if (id === 'fabric') return allData.fabricEntries.filter(e => e.direction !== 'in');
    if (id === 'payments') return allData.payments;
    if (id === 'expenses') return allData.expenses;
    return [];
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      if (section === 'yarn') { await api.deleteYarnEntry(id); allData.setYarnEntries(prev => prev.filter(e => e.id !== id)); }
      if (section === 'fabric') { await api.deleteFabricEntry(id); allData.setFabricEntries(prev => prev.filter(e => e.id !== id)); }
      if (section === 'payments') { await api.deletePayment(id); allData.setPayments(prev => prev.filter(e => e.id !== id)); }
      if (section === 'expenses') { await api.deleteExpense(id); allData.setExpenses(prev => prev.filter(e => e.id !== id)); }
    } catch (e) {
      alert('Delete failed: ' + e.message);
    }
  };

  const rows = rowsFor(section).slice().sort((a, b) => (b.date || '').localeCompare(a.date || ''));

  return (
    <div className="rf-card">
      <h3 className="rf-section-title">Edit or Delete Past Entries</h3>
      <div className="rf-tabs" style={{ marginTop: 0 }}>
        {sections.map(s => (
          <button key={s.id} className={`rf-tab ${section === s.id ? 'active' : ''}`} onClick={() => setSection(s.id)}>{s.label}</button>
        ))}
      </div>
      {rows.length === 0 ? <EmptyState text="No entries in this section yet." /> : (
        <div className="rf-table-wrap">
          <table className="rf-table">
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id}>
                  <td>{fmtDate(r.date)}</td>
                  <td>{r.party || r.category || '—'} {r.quality ? `· ${r.quality}` : ''}</td>
                  <td>{r.qty ? `${num(r.qty)} ${r.unit}` : money(r.amount)}</td>
                  <td style={{ display: 'flex', gap: 4 }}>
                    <button className="rf-icon-btn" onClick={() => setEditing({ type: section, row: r })}><Plus size={15} style={{ transform: 'rotate(45deg)' }} /></button>
                    <button className="rf-icon-btn" onClick={() => handleDelete(r.id)}><Trash2 size={15} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <Modal title="Edit Entry" onClose={() => setEditing(null)}>
          <EditEntryForm
            ...
            onSave={async (updated) => {
              try {
                if (editing.type === 'yarn') { const r = await api.updateYarnEntry(editing.row.id, updated); allData.setYarnEntries(prev => prev.map(e => e.id === r.id ? { ...e, ...updated } : e)); }
                if (editing.type === 'fabric') { const r = await api.updateFabricEntry(editing.row.id, updated); allData.setFabricEntries(prev => prev.map(e => e.id === r.id ? { ...e, ...updated } : e)); }
                if (editing.type === 'payments') { const r = await api.updatePayment(editing.row.id, updated); allData.setPayments(prev => prev.map(e => e.id === r.id ? { ...e, ...updated } : e)); }
                if (editing.type === 'expenses') { const r = await api.updateExpense(editing.row.id, updated); allData.setExpenses(prev => prev.map(e => e.id === r.id ? { ...e, ...updated } : e)); }
                setEditing(null);
              } catch (e) {
...
function EditEntryForm({ entry, type, onSave }) {
  ...
  return (
    <form onSubmit={submit} className="rf-form">
      {('party' in form) && (
        <Field label="Party"><input value={form.party || ''} onChange={e => set('party', e.target.value)} /></Field>
      )}
      {('category' in form) && (
        <Field label="Category"><input value={form.category || ''} onChange={e => set('category', e.target.value)} /></Field>
      )}
      {('quality' in form) && (
        <Field label="Quality"><input value={form.quality || ''} onChange={e => set('quality', e.target.value)} /></Field>
      )}
      {('qty' in form) && (
        <div className="rf-form-row">
          <Field label="Quantity"><input type="number" step="0.01" value={form.qty} onChange={e => set('qty', Number(e.target.value))} /></Field>
      ...
      {('amount' in form) && (
        <Field label="Amount"><input type="number" step="0.01" value={form.amount} onChange={e => set('amount', Number(e.target.value))} /></Field>
      )}
      <Field label="Date"><input type="date" value={form.date || ''} onChange={e => set('date', e.target.value)} /></Field>
      <Field label="Note"><input value={form.note || ''} onChange={e => set('note', e.target.value)} /></Field>
      <button type="submit" className="rf-btn-primary rf-form-submit">Save Changes</button>
    </form>
  );
}

function SettingsAppInfo({ allData }) {
  const totalRows = allData.yarnEntries.length + allData.production.length + allData.fabricEntries.length
    + allData.outlets.length + allData.outletStockMoves.length + allData.outletSales.length
    + allData.payments.length + allData.expenses.length;

  return (
    <div className="rf-card">
      <h3 className="rf-section-title">App &amp; Database Info</h3>
      <div className="rf-report-grid">
        <ReportBlock title="Application">
          <RLine label="App Name" value="Royal Fabrics Enterprise ERP" />
          <RLine label="Version" value="1.0.0" />
          <RLine label="Domain" value="royalfabrica.com" />
        </ReportBlock>
        <ReportBlock title="Database">
          <RLine label="Provider" value="Supabase (PostgreSQL)" />
          <RLine label="Total Records" value={totalRows} bold />
          <RLine label="Tables" value="8 active" />
        </ReportBlock>
      </div>
      <p className="rf-hint" style={{ marginTop: 16 }}>
        Designed &amp; created by Nouman Khan · 0304 9949993
      </p>
    </div>
  );
}
...
body { margin: 0; }
.rf-shell { min-height: 100vh; background: #F7F7F5; color: #1f2937; font-family: 'Inter', system-ui, sans-serif; }
...
.rf-hamburger span { width: 20px; height: 2px; background: #0A0A0A; border-radius: 2px; }
.rf-topnav-search {
  display: flex; align-items: center; gap: 8px; flex: 1; max-width: 360px;
  background: #F7F7F5; border: 1px solid #ECECEC; border-radius: 10px; padding: 8px 12px;
  color: #9ca3af;
}
.rf-topnav-search input { border: none; background: none; outline: none; font-size: 14px; width: 100%; color: #374151; }
.rf-topnav-actions { display: flex; align-items: center; gap: 8px; margin-left: auto; }
.rf-topnav-icon {
  background: none; border: none; padding: 8px; border-radius: 10px; cursor: pointer;
  color: #6b7280; display: flex; align-items: center; transition: background 0.15s, color 0.15s;
}
.rf-topnav-icon:hover { background: #F7F7F5; color: #0A0A0A; }
.rf-topnav-profile {
  display: flex; align-items: center; gap: 8px; background: #F7F7F5; border: 1px solid #ECECEC;
  border-radius: 10px; padding: 7px 12px; cursor: pointer; color: #0A0A0A; font-size: 13px; font-weight: 600;
  transition: background 0.15s;
}
.rf-topnav-profile:hover { background: #ECECEC; }
.rf-topnav-role { white-space: nowrap; }

/* ---------- SIDEBAR ---------- */
.rf-body { display: flex; }

.rf-sidebar {
  width: 220px; flex-shrink: 0; background: #0A0A0A;
  min-height: calc(100vh - 61px);
  padding: 18px 12px;
  display: flex; flex-direction: column; gap: 0;
}
.rf-sidebar-brand { display: flex; align-items: center; gap: 10px; padding: 4px 8px 14px; }
.rf-sidebar-logo { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; box-shadow: 0 0 0 1px rgba(200,162,90,0.4); }
.rf-sidebar-brand-text { display: flex; flex-direction: column; }
.rf-sidebar-name { font-family: 'Playfair Display', serif; font-weight: 700; font-size: 16px; color: #F7F7F5; line-height: 1.1; }
.rf-sidebar-tag { font-size: 10.5px; color: #C8A25A; letter-spacing: 0.06em; text-transform: uppercase; font-weight: 600; }
.rf-sidebar-divider { height: 1px; background: rgba(255,255,255,0.08); margin: 10px 4px; }
.rf-sidebar-items { display: flex; flex-direction: column; gap: 3px; flex: 1; overflow-y: auto; }

.rf-nav-item {
  display: flex; align-items: center; gap: 10px;
  padding: 10px 12px; border-radius: 10px; border: none; background: none;
  color: #9a9a9a; font-size: 13.5px; font-weight: 500; cursor: pointer; text-align: left;
  transition: background 0.15s, color 0.15s; position: relative;
}
.rf-nav-item:hover { background: rgba(255,255,255,0.06); color: #F7F7F5; }
.rf-nav-item.active {
  background: rgba(200,162,90,0.14); color: #C8A25A; font-weight: 600;
  box-shadow: inset 3px 0 0 0 #C8A25A;
}

.rf-sidebar-user { display: flex; align-items: center; gap: 10px; padding: 10px 8px 4px; color: #9a9a9a; }
.rf-sidebar-user-role { font-size: 13px; font-weight: 600; color: #F7F7F5; }
.rf-sidebar-user-sub { font-size: 11px; color: #6b6b6b; }

.rf-main { flex: 1; padding: 24px; min-width: 0; }

.rf-page-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; flex-wrap: wrap; gap: 10px; }
.rf-page-head h2 { font-family: 'Playfair Display', serif; font-size: 24px; font-weight: 700; margin: 0; color: #0A0A0A; }
...
.rf-btn-primary { background: #C8A25A; color: #0A0A0A; }
.rf-btn-secondary { background: #F3E9D2; color: #8a6d2f; }
.rf-btn-outline { background: #fff; color: #374151; border: 1px solid #ECECEC; }
...
.rf-card { background: #fff; border-radius: 20px; padding: 20px 22px; box-shadow: 0 8px 30px rgba(10,10,10,0.04); border: 1px solid #ECECEC; }
.rf-section-title { font-size: 15px; font-weight: 700; margin: 0 0 12px; color: #0A0A0A; }
...
.rf-stat-card { border-left: 4px solid #C8A25A; padding: 16px 18px; border-radius: 20px; }
...
.rf-stat-value { font-size: 22px; font-weight: 800; color: #0A0A0A; }
...
@media (max-width: 880px) {
  ...
  .rf-topnav-search { display: none; }
  .rf-sidebar {
    position: fixed; top: 57px; left: 0; bottom: 0; z-index: 50;
    transform: translateX(-100%); transition: transform 0.2s ease; box-shadow: 4px 0 14px rgba(0,0,0,0.3);
  }
...
.rf-footer {
  ...
  background: #F7F7F5; letter-spacing: 0.01em;
}
.rf-footer span { color: #C8A25A; font-weight: 700; }

.rf-settings-grid { display: flex; flex-direction: column; gap: 16px; max-width: 560px; }
.rf-settings-error { background: #fee2e2; color: #b91c1c; font-size: 12.5px; padding: 9px 11px; border-radius: 8px; }
.rf-settings-success { background: #dcfce7; color: #15803d; font-size: 12.5px; padding: 9px 11px; border-radius: 8px; }
.rf-staff-role-select { padding: 6px 8px; border-radius: 6px; border: 1px solid #ECECEC; font-size: 13px; background: #fff; }

@media print {
  .rf-topnav, .rf-sidebar, .rf-btn-row, .rf-btn-outline, .rf-icon-btn, .rf-footer { display: none !important; }
  .rf-main { padding: 0; }
